"""Convert embedding vectors into pseudo-image tensors."""
...
class Embed2Image(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        """Embed2Image constructor."""

        super().__init__()
        if channel_mode not in {"replicate", "split"}:
            raise ValueError(
                "channel_mode must be either 'replicate' or 'split'"
            )
    ...
    def forward(self, x: torch.Tensor, *, clip: bool = False) -> torch.Tensor:
        """Convert a batch of embeddings into 3-channel pseudo-images."""

        if x.ndim != 2:
            raise ValueError("Input embeddings must be 2D (batch, dim)")
        ...
        if self.channel_mode == "split":
            img = x.view(batch_size, 3, side, side).contiguous()
        else:
            img = x.view(batch_size, 1, side, side).contiguous()
