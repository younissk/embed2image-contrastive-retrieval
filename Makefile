...
.PHONY: test prepare download-dataset check-dataset train wandb-login
...
wandb-login:
	@python -c "import wandb; import sys; sys.exit(0) if wandb.Api().api_key else sys.exit(1)" || (echo 'wandb not logged in. Please login:' && wandb login)
